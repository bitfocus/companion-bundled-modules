"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var ip_regex_1 = require("ip-regex");
var IPv6 = ip_regex_1.v6({ exact: true });
/**
 * Representation of a host and a port. Supports some basic parsing via
 * `fromString(input)` supporting values such as `example.com`, `example.com:80`
 * `10.0.0.1`, `10.0.0.1:80`, `2001:db8::1` or `[2001:db8::1]:80`.
 */
var HostAndPort = /** @class */ (function () {
    function HostAndPort(host, port) {
        this.host = host;
        this.port = port;
    }
    Object.defineProperty(HostAndPort.prototype, "hasPort", {
        get: function () {
            return this.port >= 0;
        },
        enumerable: true,
        configurable: true
    });
    /**
     * Create an instance from a string.
     *
     * @param input
     */
    HostAndPort.fromString = function (input) {
        input = input.trim();
        if (input[0] === '[') {
            // Assume IPv6
            var closingBracket = input.lastIndexOf(']');
            if (closingBracket <= 0) {
                // No closing bracket, invalid
                return null;
            }
            var address = input.substring(1, closingBracket);
            if (!IPv6.test(address)) {
                return null;
            }
            /*
             * In the case of brackets, either the input ends after or its
             * followed by a colon and a port number.
             */
            if (input.length === closingBracket + 1) {
                return new HostAndPort(address, 0);
            }
            else if (input[closingBracket + 1] === ':') {
                var portString = input.substring(closingBracket + 2);
                var port = parseInt(portString, 10);
                return new HostAndPort(address, port);
            }
            else {
                // Invalid host and port
                return null;
            }
        }
        else {
            var lastColon = input.lastIndexOf(':');
            var hasMultipleColons = input.indexOf(':') !== lastColon;
            if (hasMultipleColons) {
                // If the input has multiple colons it should be an IPv6 address
                if (!IPv6.test(input)) {
                    return null;
                }
                return new HostAndPort(input, 0);
            }
            var address = lastColon >= 0 ? input.substring(0, lastColon) : input;
            var port = 0;
            if (lastColon >= 0) {
                var portString = input.substring(lastColon + 1);
                port = parseInt(portString, 10);
            }
            return new HostAndPort(address, port);
        }
    };
    HostAndPort.compare = function (a, b) {
        if (a.host < b.host) {
            return -1;
        }
        if (a.host > b.host) {
            return 1;
        }
        return a.port < b.port ? -1 : (a.port === b.port ? 0 : 1);
    };
    return HostAndPort;
}());
exports.HostAndPort = HostAndPort;
//# sourceMappingURL=host-and-port.js.map